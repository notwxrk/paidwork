...
async def send_video_from_channel(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    try:
        ...
        # Kanalda message borligini tekshirish
        message = await context.bot.get_chat_member(SECRET_CHANNEL_ID, context.bot.id)
        ...
        # Agar description bo'lsa, alohida yuborish
        if description:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success = add_user(user_id, user.username, user.first_name, user.last_name)
    ...
    # Agar obuna bo'lsa, video kod so'rash
    await update.message.reply_text("Video kodini kiriting:")

async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    message_text = update.message.text.strip()
    
    # Command bo'lsa, boshqa handler larga yo'naltiramiz
...
async def check_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    for channel_id, channel_username, channel_title in channels:
        try:
            ...
            logger.error(f"Check subscription error for channel {channel_username}: {e}")
            not_subscribed.append((channel_username, channel_title))
