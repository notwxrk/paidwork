...
async def create_tables():
    ...
    try:
        ...
        await conn.execute(
            'INSERT INTO users (user_id, phone_number, is_admin, cash_points) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id) DO UPDATE SET is_admin = $3',
            ADMIN_ID, 'admin', True, 30000
        )
...
# YANGI: TO'G'RI REFERAL BONUS FUNKSIYASI
async def give_referral_bonus(referrer_id: int, referred_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Referal bonusini to'g'ri berish"""
    conn = await init_db()
    try:
        # Avvalo, referal mavjudligini tekshiramiz
...
async def create_user(user_id: int, phone_number: str = None, referred_by: int = None, context: ContextTypes.DEFAULT_TYPE = None):
    ...
    try:
        ...
        # YANGI: Agar referred_by bo'lsa, referal yozish
        if referred_by:
...
# YANGI: TEKSHIRISH VA ASOSIY MENYUGA O'TISH FUNKSIYASI
async def check_and_show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    """Kanal tekshirish va menu ko'rsatish"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user:
        # Agar user mavjud bo'lsa, tekshirish va menu ko'rsatish
        return await check_and_show_menu(update, context, user_id)
    
    # Yangi user uchun contact so'rash
...
# YANGI: TEKSHIRISH CALLBACK - MUAMMONI TUZATILGAN
