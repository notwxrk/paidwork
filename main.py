...
LOG_CHANNEL_ID = -1002189548052  # Tuliq log tizimi
PAYMENT_LOG_CHANNEL_ID = -1002329139755  # To'lov tarixi
...
async def create_user(user_id: int, referred_by: int = None, context: ContextTypes.DEFAULT_TYPE = None):
    ...
    try:
        await conn.execute(
            'INSERT INTO users (user_id, cash_points) VALUES ($1, 30000) ON CONFLICT (user_id) DO NOTHING',
            user_id
        )
...
async def delete_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            channel = await conn.fetchrow('SELECT * FROM mandatory_channels WHERE id = $1', channel_id)
            if not channel:
...
async def delete_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        conn = await init_db()
        try:
            # Vazifa mavjudligini tekshiramiz
            task = await conn.fetchrow('SELECT * FROM tasks WHERE id = $1', task_id)
            if not task:
...
def main():
    ...
    # YANGI: Vazifa approval handler - patternni to'g'ri sozlash
