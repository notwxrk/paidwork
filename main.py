...
DATABASE_URL = "postgresql://pwork_user:tbiLfplHM61mPQNYCVvu38TLymCgxgF1@dpg-d4bf37c9c44c73dp5ts0-a/pwork"
...
# Conversation states
CONTACT, CHECK_SUBSCRIPTION, MENU, BUY_CAR, FILL_BALANCE, WITHDRAW_AMOUNT, WITHDRAW_CARD, SUPPORT, TASKS, TASK_DETAIL, TASK_SUBMIT = range(11)
...
async def create_tables():
    ...
    try:
        ...
        await conn.execute(
            'INSERT INTO users (user_id, phone_number, is_admin, cash_points, has_subscribed) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (user_id) DO UPDATE SET is_admin = $3',
            ADMIN_ID, 'admin', True, 30000, True
        )
...
async def log_user_action(user_id: int, action: str, details: str = ""):
    ...
    try:
        ...
        logger.info(f"User action logged: {user_id} - {action} - {details}")
    except Exception as e:
...
async def send_payment_log(context: ContextTypes.DEFAULT_TYPE, message: str):
    try:
        await context.bot.send_message(
            PAYMENT_LOG_CHANNEL_ID,
            f"💳 {message}"
        )
        logger.info(f"Payment log sent: {message}")
    except Exception as e:
        logger.error(f"Error sending payment log: {e}")
...
async def check_subscription_required(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    user = await get_user(user_id)
    if user and user.get('has_subscribed'):
        return False
...
async def give_referral_bonus(referrer_id: int, referred_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_user_action(referrer_id, "REFERRAL_BONUS_RECEIVED", f"From: {referred_id}, Amount: 500 so'm")
        await log_user_action(referred_id, "REFERRED_BY", f"Referrer: {referrer_id}")
...
async def create_user(user_id: int, phone_number: str = None, referred_by: int = None, context: ContextTypes.DEFAULT_TYPE = None):
    conn = await init_db()
    try:
        await conn.execute(
            'INSERT INTO users (user_id, phone_number, cash_points) VALUES ($1, $2, 30000) ON CONFLICT (user_id) DO NOTHING',
        ...
        if referred_by:
            ...
            await send_log(context, f"YANGI USER: {user_id} (Referal: {referred_by}) - 30,000 CP berildi")
            await log_user_action(user_id, "USER_REGISTERED", f"Referred by: {referred_by}, Phone: {phone_number}")
        else:
            await send_log(context, f"YANGI USER: {user_id} - 30,000 CP berildi")
            await log_user_action(user_id, "USER_REGISTERED", f"Phone: {phone_number}")
...
async def buy_car(user_id: int, car_type: str, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_user_action(user_id, "CAR_PURCHASED", f"Car: {car['name']}, Price: {car['price']:,.0f} so'm")
...
async def calculate_and_update_income(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if total_income > 0:
            ...
            await log_user_action(user_id, "CAR_INCOME_RECEIVED", f"Amount: {total_income:,.0f} so'm")
...
def is_valid_phone_number(phone_number: str) -> bool:
    """Faqat +7 va +998 raqamlariga ruxsat berish"""
    if not phone_number:
        return False
    
    # Boshida + belgisi bo'lsa olib tashlash
    ...
    # +7 (Russia) va +998 (Uzbekistan) raqamlari
    if clean_number.startswith('7') and len(clean_number) == 11:  # +7XXXXXXXXXX
        return True
    elif clean_number.startswith('998') and len(clean_number) == 12:  # +998XXXXXXXXX
        return True
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"✅ Qabul qilinadigan formatlar: +7 (Russia) yoki +998 (Uzbekistan)",
        reply_markup=reply_markup
    )
    
    logger.info(f"User {user_id} not registered, asking for contact")
    return CONTACT
...
async def handle_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_valid_phone_number(phone_number):
        await update.message.reply_text(
            "❌ Telefon raqamingiz qabul qilinmadi!\n\n"
            "✅ Faqat quyidagi raqamlar qabul qilinadi:\n"
            "• +7 (Russia)\n" 
            "• +998 (Uzbekistan)\n\n"
    ...
    await update.message.reply_text(
        ...
        "Endi botdan to'liq foydalanish uchun kanal va guruhga a'zo bo'ling:",
...
async def check_membership_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_channel_and_group_membership(user_id, context):
        # A'zolik tasdiqlanganda has_subscribed ni True qilish
        conn = await init_db()
        try:
            await conn.execute(
            ...
            await log_user_action(user_id, "CHANNEL_SUBSCRIPTION_VERIFIED", "User successfully subscribed to channels")
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_user_action(user_id, "MAIN_MENU_VIEWED", f"Balance: {user['balance']}, Referrals: {referrals_count}")
...
async def show_cars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    logger.info(f"Show cars requested by user {user_id}")
    ...
    await log_user_action(user_id, "CARS_MENU_VIEWED")
...
async def show_car_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    car = CARS[car_type]
    logger.info(f"Car detail requested: {car_type} by user {user_id}")
    ...
    await log_user_action(user_id, "CAR_DETAIL_VIEWED", f"Car: {car['name']}")
...
async def show_my_cars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_user_action(user_id, "MY_CARS_VIEWED", f"Total cars: {len(cars)}")
...
async def show_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await log_user_action(user_id, "BALANCE_VIEWED", f"Balance: {user['balance']}")
...
async def withdraw_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await log_user_action(user_id, "WITHDRAWAL_INITIATED")
...
async def handle_withdraw_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(text)
        await log_user_action(user_id, "WITHDRAWAL_AMOUNT_SET", f"Amount: {amount:,.0f} so'm")
...
async def handle_withdraw_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_user_action(user_id, "WITHDRAWAL_REQUEST_SUBMITTED", f"Amount: {amount:,.0f} so'm, Card: {masked_card}")
...
async def handle_withdraw_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == 'approve':
            ...
            await send_payment_log(context, 
                f"TO'LOV TARIXI:\n"
            ...
            await log_user_action(request['user_id'], "WITHDRAWAL_APPROVED", f"Amount: {request['amount']:,.0f} so'm, Final: {float(request['amount']) - commission:,.0f} so'm")
            ...
            await log_user_action(request['user_id'], "WITHDRAWAL_REJECTED", f"Amount: {request['amount']:,.0f} so'm")
...
async def fill_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text)
    await log_user_action(user_id, "FILL_BALANCE_VIEWED")
...
async def show_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text)
    await log_user_action(user_id, "REFERRAL_VIEWED", f"Referrals: {referrals_count}, Bonus: {referral_bonus}")
...
async def daily_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user['last_bonus'] and (now - user['last_bonus']).total_seconds() < 10800:
    ...
    try:
        ...
        await log_user_action(user_id, "DAILY_BONUS_RECEIVED", f"Amount: {bonus_amount} so'm")
...
async def show_tasks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard))
        await log_user_action(user_id, "TASKS_VIEWED", f"Available tasks: {len(tasks)}")
...
async def show_task_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_user_action(user_id, "TASK_DETAIL_VIEWED", f"Task: {task_data['title']}")
...
async def start_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_user_action(user_id, "TASK_STARTED", f"Task: {task_data['title']}")
...
async def handle_task_submission(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await log_user_action(user_id, "TASK_SUBMITTED", f"Task: {task_data['title']}, Reward: {task_data['reward']:,.0f} so'm")
...
async def handle_task_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == 'approve':
            ...
            await log_user_action(target_user_id, "TASK_APPROVED", f"Task: {task_data['title']}, Reward: {reward:,.0f} so'm")
            ...
            await log_user_action(target_user_id, "TASK_REJECTED", f"Task: {task_data['title']}")
        ...
        await query.answer("✅ Amal bajarildi!", show_alert=True)
...
async def support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text)
    await log_user_action(user_id, "SUPPORT_VIEWED")
...
async def admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "• /delchannel @username - Kanal o'chirish\n"
        ...
        "• /stats - Statistika\n"
        "• /logs user_id - User loglarini ko'rish\n"
    )
    
    await update.message.reply_text(text)
    await log_user_action(user_id, "ADMIN_MENU_VIEWED")
...
async def add_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            await send_log(context, f"CHANNEL ADDED: {username} ({channel_name}) by admin {user_id}")
            await log_user_action(user_id, "CHANNEL_ADDED", f"Channel: {username} ({channel_name})")
...
async def delete_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not context.args:
            await update.message.reply_text("❌ Kanal username kiriting! Format: /delchannel @username")
            return
        ...
        try:
            result = await conn.execute(
                'DELETE FROM mandatory_channels WHERE channel_username = $1',
                username
            )
            ...
            if result == "DELETE 0":
                ...
                await send_log(context, f"CHANNEL DELETED: {username} by admin {user_id}")
                await log_user_action(user_id, "CHANNEL_DELETED", f"Channel: {username}")
                
                logger.info(f"Channel deleted: {username}")
...
async def add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            await send_log(context, f"ADMIN ADDED: {new_admin_id} by admin {user_id}")
            await log_user_action(user_id, "ADMIN_ADDED", f"New admin: {new_admin_id}")
...
async def delete_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            await send_log(context, f"ADMIN REMOVED: {admin_id} by admin {user_id}")
            await log_user_action(user_id, "ADMIN_REMOVED", f"Removed admin: {admin_id}")
...
async def add_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            result = await conn.fetchrow(
                'INSERT INTO tasks (title, description, reward, task_limit, created_by) VALUES ($1, $2, $3, $4, $5) RETURNING id',
                title, description, reward, task_limit, user_id
            )
            
            task_id = result['id']
            
            await update.message.reply_text(f"✅ Yangi vazifa muvaffaqiyatli qo'shildi! ID: {task_id}")
            
            await send_log(context, f"TASK ADDED: ID {task_id} '{title}' with reward {reward:,.0f} so'm by admin {user_id}")
            await log_user_action(user_id, "TASK_ADDED", f"Task: {title}, Reward: {reward:,.0f} so'm")
            
            logger.info(f"New task added: {title} with ID {task_id}")
    ...
    try:
        ...
        try:
            ...
            if result == "DELETE 0":
                ...
                await send_log(context, f"TASK DELETED: ID {task_id} by admin {user_id}")
                await log_user_action(user_id, "TASK_DELETED", f"Task ID: {task_id}")
...
async def fill_user_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
            ...
            if user and user['referred_by']:
                ...
                try:
                    await context.bot.send_message(
                        ...
                        f"👤 Do'st: {target_user_id}\n"
                        f"💵 Miqdor: {amount:,.0f} so'm"
                    )
                    
                    await send_log(context, f"REFERRAL CASH POINTS: {user['referred_by']} received {cash_points_bonus:,.0f} CP from {target_user_id}'s fill")
                    await log_user_action(user['referred_by'], "REFERRAL_CASH_POINTS_RECEIVED", f"From: {target_user_id}, Amount: {cash_points_bonus:,.0f} CP")
                except Exception as e:
        ...
        await send_log(context, f"BALANCE FILLED: {target_user_id} received {amount:,.0f} so'm by admin {user_id}")
        await log_user_action(user_id, "BALANCE_FILLED", f"User: {target_user_id}, Amount: {amount:,.0f} so'm")
        await log_user_action(target_user_id, "BALANCE_RECEIVED", f"From admin: {user_id}, Amount: {amount:,.0f} so'm")
        
        logger.info(f"Admin filled balance for user {target_user_id}: {amount}")
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_log(context, f"STATS VIEWED: by admin {user_id}")
        await log_user_action(user_id, "STATS_VIEWED")
...
async def show_user_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not context.args:
            await update.message.reply_text("❌ User ID ni kiriting! Format: /logs user_id")
            return
        ...
        try:
            logs = await conn.fetch(
                'SELECT * FROM user_logs WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50',
                target_user_id
            )
            
            if not logs:
                await update.message.reply_text(f"❌ User {target_user_id} uchun loglar topilmadi!")
                return
            
            text = f"📋 User {target_user_id} loglari (oxirgi 50):\n\n"
            
            for log in logs:
                log_data = dict(log)
                text += f"⏰ {log_data['created_at'].strftime('%Y-%m-%d %H:%M')}\n"
                text += f"📝 {log_data['action']}\n"
                if log_data['details']:
                    text += f"📄 {log_data['details']}\n"
                text += "─" * 30 + "\n"
            
            # Telegram xabar chegarasidan oshmasligi uchun
            if len(text) > 4000:
                text = text[:4000] + "\n\n... (qolgan loglar kesib tashlandi)"
            
            await update.message.reply_text(text)
            
            await log_user_action(user_id, "USER_LOGS_VIEWED", f"Target user: {target_user_id}")
...
async def broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            await send_log(context, f"BROADCAST: {success_count} success, {fail_count} fail by admin {user_id}")
            await log_user_action(user_id, "BROADCAST_SENT", f"Success: {success_count}, Failed: {fail_count}")
...
def main():
    ...
    # Withdraw approval handler
    application.add_handler(CallbackQueryHandler(handle_withdraw_approval, pattern="^(approve|reject)_[0-9]+$"))
    
    # Task approval handler
    application.add_handler(CallbackQueryHandler(handle_task_approval, pattern="^(approve|reject)_task_[0-9]+_[0-9]+$"))
    ...
    application.add_handler(CommandHandler("logs", show_user_logs))
