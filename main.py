...
# YANGI: Referal bonus berish funksiyasi - KUNLIK BONUS UCHUN
...
# YANGI: Kunlik bonus uchun referal bonus
async def give_daily_bonus_referral(referred_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Kunlik bonus olinganda referalga bonus berish"""
    conn = await init_db()
    try:
        # Referalni topamiz
        referral = await conn.fetchrow(
            'SELECT * FROM referrals WHERE referred_id = $1 AND bonus_paid = TRUE',
            referred_id
        )
        ...
        if referral:
            ...
            # 100 so'm bonus
            await conn.execute(
                'UPDATE users SET balance = balance + 100, referral_bonus_earned = referral_bonus_earned + 100 WHERE user_id = $1',
                referrer_id
            )
            ...
            try:
                await context.bot.send_message(
                    referrer_id,
                    "🎉 Tabriklaymiz! Taklif qilgan do'stingiz kunlik bonus oldi!\n\n"
                    "💰 +100 so'm bonus qo'shildi!\n"
                    "👥 Do'stingiz kunlik bonusni muvaffaqiyatli oldi!"
                )
            except Exception as e:
                logger.error(f"Error sending daily bonus referral message to {referrer_id}: {e}")
            
            logger.info(f"Daily bonus referral paid: {referrer_id} -> {referred_id}: 100 so'm")
        
    except Exception as e:
        logger.error(f"Error giving daily bonus referral: {e}")
    finally:
        await conn.close()
...
async def show_cars(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # YANGI: 2 qatorli keyboard
    keyboard = [
        [InlineKeyboardButton("BMW", callback_data="car_bmw"), InlineKeyboardButton("Mercedes", callback_data="car_mercedes")],
        [InlineKeyboardButton("Nissan", callback_data="car_nissan"), InlineKeyboardButton("Supra", callback_data="car_supra")],
...
async def show_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    can_withdraw = float(user['balance']) >= 20000 and float(user['cash_points']) >= 20000
...
async def withdraw_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Tekshiramiz, cash points bormi - YANGI: 20,000 CP talab qilinadi
...
async def handle_withdraw_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Tekshiramiz, cash points yetarlimi - YANGI: 20,000 CP talab qilinadi
        if float(user['cash_points']) < 20000:
...
async def show_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• Do'stingiz kunlik bonus olsa: 100 so'm bonus\n"
...
async def daily_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # YANGI: Kunlik bonus uchun referal bonus
        await give_daily_bonus_referral(user_id, context)
...
async def delete_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not context.args:
            await update.message.reply_text("❌ Kanal ID sini kiriting! Format: /delete_channel id")
            return
...
async def remove_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not context.args:
            await update.message.reply_text("❌ User ID ni kiriting! Format: /remove_admin user_id")
            return
