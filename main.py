    ...
    try:
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS users (
                ...
                free_withdraw_used BOOLEAN DEFAULT FALSE,
...
async def check_subscription_required(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    return not await check_channel_membership(user_id, context)
...
async def give_referral_bonus(referrer_id: int, referred_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # 500 so'm bonus berish
        await conn.execute(
            'UPDATE users SET balance = balance + 500, referral_bonus_earned = referral_bonus_earned + 500, referral_count = referral_count + 1 WHERE user_id = $1',
            referrer_id
        )
...
async def create_user(user_id: int, phone_number: str = None, referred_by: int = None, context: ContextTypes.DEFAULT_TYPE = None):
    ...
    try:
        await conn.execute(
            'INSERT INTO users (user_id, phone_number) VALUES ($1, $2) ON CONFLICT (user_id) DO NOTHING',
...
async def can_user_withdraw(user_id: int) -> tuple:
    """Foydalanuvchi pul yechish huquqini tekshiradi"""
    user = await get_user(user_id)
    if not user:
        return False, "Foydalanuvchi topilmadi"
    
    # 1-marta bepul
    if not user['free_withdraw_used']:
        return True, "free"
    
    # Keyingi marta uchun 5 ta taklif talab qilinadi
    if user['referral_count'] >= 5:
        return True, "referral"
    
    return False, f"Pul yechish uchun {5 - user['referral_count']} ta do'st taklif qilishingiz kerak"

async def use_free_withdraw(user_id: int):
    """Bepul pul yechish imkoniyatini ishlatadi"""
    conn = await init_db()
    try:
        await conn.execute(
            'UPDATE users SET free_withdraw_used = TRUE WHERE user_id = $1',
            user_id
        )
        logger.info(f"Free withdraw used by user {user_id}")
    except Exception as e:
        logger.error(f"Error updating free withdraw for user {user_id}: {e}")
    finally:
        await conn.close()
...
async def check_membership_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_channel_membership(user_id, context):
        # Kanal tasdiqlanganda referal bonus berish
        user = await get_user(user_id)
        if user and user['referred_by']:
            await give_referral_bonus(user['referred_by'], user_id, context)
        
        await query.edit_message_text("✅ Siz kanallarga a'zo bo'lgansiz! Endi botdan to'liq foydalanishingiz mumkin.")
        await show_main_menu(update, context)
        return MENU
    else:
        await query.answer("Siz hali kanallarga a'zo bo'lmagansiz!", show_alert=True)
...
# Balance section - BU FUNKSIYA MUAMMOSINI TUZATAMIZ
@check_subscription
async def show_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user = await get_user(user_id)
    
    can_withdraw, withdraw_type = await can_user_withdraw(user_id)
    
    text = (
        f"💸 Hisobim\n\n"
        f"💰 Joriy balans: {float(user['balance']):,.0f} so'm\n"
        f"📈 Umumiy daromad: {float(user['total_earned']):,.0f} so'm\n"
        f"🚗 Faol mashinalar: {len(await get_user_cars(user_id))} ta\n"
        f"👥 Taklif qilgan do'stlar: {user['referral_count']} ta"
    )
    
    keyboard = [[InlineKeyboardButton("💳 Pul yechish", callback_data="withdraw")]]
    
    if can_withdraw:
        if withdraw_type == "free":
            text += f"\n\n💳 Sizda 1-marta BEPUL pul yechish imkoniyati mavjud!"
        else:
            text += f"\n\n💳 Pul yechish mumkin ({user['referral_count']}/5 taklif)"
        text += f"\n📉 Komissiya: 15%"
    else:
        text += f"\n\n⚠️ Pul yechish uchun {5 - user['referral_count']} ta do'st taklif qilishingiz kerak!"
...
async def withdraw_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_withdraw:
        await query.answer(f"❌ Pul yechish uchun {5 - user['referral_count']} ta do'st taklif qilishingiz kerak!", show_alert=True)
        return
    
    text = (
        f"💳 Pul yechish\n\n"
        f"💰 Mavjud balans: {float(user['balance']):,.0f} so'm\n"
        f"💸 Minimal yechish: 20,000 so'm\n"
        f"📉 Komissiya: 15%\n"
    )
    
    if withdraw_type == "free":
        text += f"\n🎁 1-MARTA BEPUL PUL YECHISH!\n"
    else:
        text += f"\n✅ Pul yechish mumkin ({user['referral_count']}/5 taklif)\n"
    
    text += f"\nYechish uchun miqdorni kiriting (so'mda):"
...
async def handle_withdraw_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    user = await get_user(user_id)
    withdraw_type = context.user_data.get('withdraw_type', 'referral')
    ...
    try:
        ...
        # Bepul yechish imkoniyatini ishlatish
        if withdraw_type == "free":
            await use_free_withdraw(user_id)
...
async def handle_withdraw_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        withdraw_type_text = "BEPUL (1-marta)" if withdraw_type == "free" else f"TAKLIF ({user['referral_count']}/5)"
        ...
        await context.bot.send_message(
            ...
            f"🎁 Turi: {withdraw_type_text}\n"
            f"⏰ Vaqt: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
            f"Pul tushirildimi?",
            reply_markup=reply_markup
        )
        
        await send_log(context, f"WITHDRAW REQUEST: {user_id} requested {amount:,.0f} so'm to card {masked_card} ({withdraw_type_text})")
...
async def handle_withdraw_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == 'approve':
            ...
            await conn.execute(
                'UPDATE users SET balance = balance - $1 WHERE user_id = $3',
...
# Referral section - BU FUNKSIYANI HAM TUZATAMIZ
@check_subscription
async def show_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• 5 ta do'st taklif qilganingizdan so'ng pul yechishingiz mumkin\n"
    )
    ...
    try:
        ...
        try:
            ...
            if result == "DELETE 0":
                ...
                await send_log(context, f"CHANNEL DELETED: {username} by admin {update.effective_user.id}")
...
async def add_task(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            await send_log(context, f"TASK ADDED: ID {task_id} '{title}' with reward {reward:,.0f} so'm by admin {update.effective_user.id}")
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        total_referrals = await conn.fetchval('SELECT COUNT(*) FROM referrals WHERE bonus_paid = TRUE')
